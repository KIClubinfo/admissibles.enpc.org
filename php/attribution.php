...
while($demande = $liste_demandes_loin->fetch_assoc()){   //On regarde 1 à 1 les demandes
    ...
    while($room = $voeu1->fetch_assoc() and $cherche){ //Cycle sur les chambres du bon type -> stock dans $room
        ...
        $simple_pleine = $occupe -> num_rows > 0 and $room[$FIELD_TYPE]==$SIMPLE; //simple remplie
        $deux_personnes = $binome2 != NULL;//deux personnes
        $incompatible_gender = $binome1 == NULL or $binome1[$FIELD_GENDER] == $demande[$FIELD_GENDER]; //Genre incompatible
        $chambre_valide = $binome1 == NULL or !($simple_pleine or $deux_personnes or $incompatible_gender);
        $heure_arrivee = "";
        for($i=0; $i<strlen($demande[$FIELD_ARRIVAL_TIME])+3; $i++) {
            ...
            if ($i == 1 or $i == strlen($demande[$FIELD_ARRIVAL_TIME])) {
        ...
        for($i=0; $i<strlen($demande[$FIELD_DEPARTURE_TIME])+3; $i++) {
            ...
            if ($i == 1 or $i == strlen($demande[$FIELD_DEPARTURE_TIME])) {
